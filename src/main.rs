...
pub mod connection
{
    ...
    pub fn run_server()
    {
        ...
        loop {
            ...
            // Time can be reduced if you have a lot of FPS. I have 60, I set it to 16.6 ms
            if timer.elapsed().as_millis() > 16 {
                crate::input::mouse::send_move(x, y); // send to OS
                timer = Instant::now();
            }
        }
    }
    ...
    pub fn send_move(socket: &UdpSocket, x: i32, y: i32)
    {
        ...
        socket.send_to(&arr, "127.0.0.1:228").unwrap(); // sends to the program
    }
}
